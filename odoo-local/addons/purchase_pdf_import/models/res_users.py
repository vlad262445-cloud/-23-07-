...
class ResUsers(models.Model):
    ...
    # Переименовано из department_id (2026-07-24) - штатный модуль hr
    # (устанавливается модулем hr_workwear) заводит своё res.users.
    # department_id (related на hr.employee.department_id, hr.department) -
    # при совпадении имени поля Odoo молча склеивает атрибуты ДВУХ разных
    # определений в одно битое поле (comodel_name отсюда, related/store от
    # hr), что падает при первом же обращении. Одноимённые поля с разными
    # comodel_name на одной модели через _inherit - в принципе не
    # поддерживаемая Odoo комбинация, а не что-то, что можно настроить.
    # Старые данные (5 пользователей) переносятся сырым SQL в
    # hr_workwear.__init__.post_init_hook - именно этот модуль вводит
    # зависимость от hr и обнажает конфликт, поэтому миграция живёт там.
    purchase_department_id = fields.Many2one(
        'purchase.request.department', string='Отдел/участок (заявки)',
        help='"Домашний" отдел сотрудника для заявок на закупку. Если указан, '
             'обычный сотрудник (без роли Закупщика/Главного закупщика) сможет '
             'подавать заявки только по этому отделу - поле "Отдел/участок" в '
             'заявке будет заблокировано на этом значении. Закупщика/менеджера '
             'это не ограничивает.')
